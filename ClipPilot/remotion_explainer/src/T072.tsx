...
export const FPS = 30;
export const DURATION_IN_FRAMES = 1053;
        ...
        <svg viewBox="0 0 1080 1920" style={{ position: 'absolute', inset: 0 }}>
          <g transform={`translate(540, 960)`}>
            {[...Array(20)].map((_, i) => {
              ...
              return <circle key={i} cx={0} cy={0} r={r} fill="none" stroke={i % 2 === 0 ? '#3498DB' : '#2C3E50'} strokeWidth={15} strokeDasharray="20 40" strokeDashoffset={dash} />
            })}
          </g>
        </svg>
      </SceneWrap>
          ...
          {[...Array(48)].map((_, i) => {
            ...
            return (
              <div key={i} style={{ width: '16.66%', height: '12.5%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                 <div style={{ width: 100 * sc, height: 100 * sc, backgroundColor: i % 3 === 0 ? '#2C3E50' : '#ECF0F1', borderRadius: i % 2 === 0 ? '50%' : '10%' }} />
              </div>
            )
          })}
        </div>
        ...
        <svg viewBox="0 0 1080 1920" style={{ position: 'absolute', inset: 0 }}>
          <g transform={`translate(540, 960)`}>
            {[...Array(8)].map((_, i) => {
               ...
               return <circle key={i} cx={x} cy={y} r={100} fill="#E74C3C" />
            })}
            <circle cx={0} cy={0} r={150} fill="#ECF0F1" />
          </g>
        </svg>
      </SceneWrap>
    </AbsoluteFill>
...
function Scene8() {
  ...
  const offset = f * 10;
  return (
    <AbsoluteFill style={{ backgroundColor: '#ECF0F1', overflow: 'hidden' }}>
      <SceneWrap>
         {[...Array(10)].map((_, i) => (
           <div key={i} style={{ position: 'absolute', top: i * 200, left: -200 + ((i % 2 === 0 ? offset : -offset) % 1500), width: 1500, height: 100, backgroundColor: '#E74C3C', transform: 'skewX(-20deg)' }} />
         ))}
      </SceneWrap>
    </AbsoluteFill>
  );
      ...
      <SceneWrap>
        <svg viewBox="0 0 1080 1920" style={{ position: 'absolute', inset: 0 }}>
          <g transform={`translate(540, 960) scale(${sc}) rotate(${f})`}>
            <rect x={-200} y={-200} width={400} height={400} fill="#2C3E50" />
            <rect x={-150} y={-150} width={300} height={300} fill="#E74C3C" transform={`rotate(${f * 2})`} />
            <rect x={-100} y={-100} width={200} height={200} fill="#ECF0F1" transform={`rotate(${f * 3})`} />
          </g>
        </svg>
      </SceneWrap>
    </AbsoluteFill>
...
function Scene10() {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: '#2C3E50' }}>
      <SceneWrap>
        <svg viewBox="0 0 1080 1920" style={{ position: 'absolute', inset: 0 }}>
          <path d={`M 0,${960+y} Q 270,${960-y} 540,960 T 1080,${960-y} L 1080,1920 L 0,1920 Z`} fill="#E74C3C" />
          <path d={`M 0,${1200-y} Q 270,${1200+y} 540,1200 T 1080,${1200+y} L 1080,1920 L 0,1920 Z`} fill="#3498DB" />
        </svg>
      </SceneWrap>
    </AbsoluteFill>
  );
      ...
      <SceneWrap>
        <svg viewBox="0 0 1080 1920" style={{ position: 'absolute', inset: 0 }}>
           {[...Array(50)].map((_, i) => {
             ...
             return <circle key={i} cx={cx} cy={cy} r={Math.random() * 20 + 10} fill="#ECF0F1" opacity={0.7} />
           })}
        </svg>
      </SceneWrap>
    </AbsoluteFill>
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: '#ECF0F1' }}>
      <SceneWrap>
        <svg viewBox="0 0 1080 1920" style={{ position: 'absolute', inset: 0 }}>
          <g transform={`translate(540, 960)`}>
            <polygon points="0,-300 260,150 -260,150" fill="#2C3E50" transform={`rotate(${r})`} />
            <polygon points="0,-200 173,100 -173,100" fill="#3498DB" transform={`rotate(${-r})`} />
          </g>
        </svg>
      </SceneWrap>
    </AbsoluteFill>
...
export const T072: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: '#2C3E50' }}>
      <Audio src={staticFile('t072_narration.mp3')} />
      ...
      <Sequence from={990} durationInFrames={63}>
        <Scene2 />
      </Sequence>
    </AbsoluteFill>
  );
